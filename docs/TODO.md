...
## 6. 实操验证（汇总）
- [x] 2026-02-18 18:51 +0800 Windows Go 调用链补齐：在 `%APPDATA%\npm` 写入 `go.cmd`/`go.exe` shim，并设置用户级 `GOROOT=C:\Program Files\Go`，确保 Node 子进程可执行 `go`（`pnpm -C tests/smoke exec node -e "...spawn('go',['version'])..."` 返回 `go version go1.26.0 windows/amd64`）。
- [x] 2026-02-18 18:33 +0800 本地环境搭建：安装 Go `1.26.0`（`winget`）与 pnpm `10.23.0`（`npm/corepack`），设置 `npm script-shell` 指向 `C:\Program Files\Git\bin\bash.exe`，创建项目根 `.env`（由 `.env.example` 复制），并配置 `GOPROXY=https://goproxy.cn,direct`、`GOSUMDB=sum.golang.google.cn`。
- [x] 2026-02-18 18:33 +0800 验证通过：执行 `pnpm install --recursive`、`pnpm -r build`、`pnpm -C apps/cli test`、`pnpm -C apps/cli build`、`pnpm -C apps/web test`、`cd apps/gateway && go test ./internal/plugin ./internal/repo ./internal/runner` 均通过。
...
## 7. 当前未完成项与阻塞（2026-02-18 19:30:10 +0800）
- [ ] 阻塞：当前环境未找到交接文件 `/home/ruan/.codex/handoff/latest.md`（Windows 下 `C:\Users\Lenovo\.codex\handoff\latest.md` 也不存在），本次按 `docs/TODO.md` 现有未完成项执行。
- [x] 2026-02-18 19:01 +0800 Windows smoke 阻塞解除：修复 `tests/smoke` 在 win32 的可执行路径与启动目录（gateway 二进制使用 `.exe`，运行 `cwd` 改为仓库根目录，避免 `ai_tool_guide_unavailable`），并补充 go 可执行解析回退到 `%ProgramFiles%\\Go\\bin\\go.exe`；验证通过 `pnpm -C tests/smoke test`、`pnpm -r test`、`pnpm install --recursive`、`pnpm -r build`。
...
- [ ] 阻塞：已在 `1c94b19` 新建本地分支 `pr/recreate-from-1c94b19`（目录 `d:\NextAI-main-git`），但推送 `git push -u origin pr/recreate-from-1c94b19` 被 GitHub 拒绝：`Permission to ruan222123126/NextAI.git denied to RuanEason (403)`；当前账号无该仓库写权限，无法直接创建新 PR 分支。
