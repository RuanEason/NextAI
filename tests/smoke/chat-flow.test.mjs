...
test("gateway chat flow e2e: create -> stream send -> history", { timeout: 90_000 }, async () => {
  ...
  const gatewayBin = gatewayBinaryPath(dataDir, "gateway-smoke");
...
function runCommand(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    const resolvedCommand = command === "go" ? goCommand : command;
    ...
    proc.once("exit", (code) => {
      ...
      reject(new Error(`${resolvedCommand} ${args.join(" ")} failed (${code})\n${output}`));
    });
  });
}

function gatewayBinaryPath(dir, baseName) {
  return join(dir, process.platform === "win32" ? `${baseName}.exe` : baseName);
}
...
function resolveGoCommand() {
  ...
  if (process.env.GOROOT) {
    const goInGoRoot = join(process.env.GOROOT, "bin", "go.exe");
    if (existsSync(goInGoRoot)) {
      return goInGoRoot;
    }
  }
  const goInProgramFiles = join(process.env.ProgramFiles ?? "C:\\Program Files", "Go", "bin", "go.exe");
  if (existsSync(goInProgramFiles)) {
